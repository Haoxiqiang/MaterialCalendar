...
public class CloudFogView extends View {
    ...
    public void onDraw(Canvas canvas) {
        ...
        paintCloud.setStrokeWidth((float) (0.02083 * width));
        paintFog.setStrokeWidth((float) (0.02083 * width));
        ...
        canvas.drawPath(cloud.getCloud(X, Y, width, count), paintCloud);
        PointF P1c2 = cloud.getP1c2(X, Y, width, count);

        path1 = new Path();
        path2 = new Path();

        if (line1Y == 0) {
            line1Y = P1c2.y + (float) (0.1042 * width);   //Calculating Y coordinate for foglines.
            line2Y = P1c2.y + (float) (0.15625 * width);
